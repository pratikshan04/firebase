...
function generateValidationList() {
  validationListElement =
    '<select id="validationlist"><option value="">-Select-</option>';
  for (i = 0; i < validationList.length; i++) {
    console.log(validationList[i].name);
    validationListElement =
      validationListElement +
      '<option value="' +
      validationList[i].value +
      '">' +
      validationList[i].name +
      "</option>";
  }
...
$(".apply-changes").on("click", function () {
  ...
  if (type == "radio" || type == "checkbox") {
    ...
    obj
      ...
      .attr(
        "data-type",
        $("#validationlist").val() == "" ? "text" : $("#validationlist").val()
      );
    ...
    var el =
      '<div><a href="javascirpt:void(0);" class="tool-edit">Edit</a> <a href="javascirpt:void(0);" class="tool-delete">Delete</a></div>';
    obj.append(el);
  } else if (type == "select") {
...
function buildArrayElement(names, values, name, type, obj) {
  ...
  for (i = 0; i < nameArr.length; i++) {
    ...
    if (type == "select") {
      element =
        element + '<option value="' + elValue + '">' + elName + "</option>";
    } else {
      ...
      element =
        ...
        '<label class="' +
        lableClass +
        '" style="display:block;"><input type="' +
        type +
        '" name="' +
        elementName +
        '" ' +
...
var dropOpts = {
  ...
  drop: function (event, ui) {
    $("<li title='Drag to Reorder'></li>")
      .attr("data-type", ui.draggable.data("type"))
      .html(buildElement(ui.draggable.data("type")))
      .appendTo(this);
  },
};
...
$(".dropelement form ul")
  ...
  .disableSelection()
  .bind("click.sortable mousedown.sortable", function (e) {
...
function buildElement(type) {
  ...
  if (type == "input") {
    el =
      '<div class="form-group"><label>Text Input</label><input type="text" value="" class="form-control" placeholder="Input Text"/></div>';
  } else if (type == "text") {
    el =
      '<div class="form-group"><div class="text-content">Type your text here.</div>';
  } else if (type == "textarea") {
    el =
      '<div class="form-group"><label>Text area</label><textarea class="form-control" ></textarea></div>';
  } else if (type == "select") {
    el =
      '<div class="form-group"><label>Select</label><select name="select1"  class="form-control"><option value="option1">option1</option></select></div>';
  } else if (type == "radio") {
    el =
      '<div class="form-group"><p>Radios</p><label class="customRadioBtn" style="display:block;"><input type="radio" value="radio" name="radio1" class="form-control" /><span>Radio</span></label></div>';
  } else if (type == "checkbox") {
    el =
      '<div class="form-group"><p>Checkbox</p><label class="customCheckBox" style="display:block;"><input type="checkbox" name="checkbox1" class="form-control" /><span>checkboxname</span><label></div>';
  } else if (type == "captcha") {
    var webthemesval =
      document.getElementById("webthemesval").value +
      "/CMS/images/active-icons/captcha.png";
    el =
  ...
  if (type != "captcha") {
    ...
    el =
      el +
      '<div><a href="javascirpt:void(0);" class="tool-delete">Delete</a></div>';
  }
  return el;
}
...
function buildEditForm(type, parentObj) {
  ...
  for (var key in settings.fields) {
    if (settings.fields.hasOwnProperty(key)) {
      ...
      list =
        ...
        buildEditElement(
          key,
          settings.fields[key].label,
          settings.fields[key].type,
  ...
  if (type != "text") {
    list =
      list +
      '<li><div class="form-group"><label>Validate</label>' +
      validationListElement +
      "</li></div>";
  }
  ...
  if (parentObj.find(type).attr("data-type") != "") {
    $(
      "#validationlist option[value=" +
        parentObj.find(type).attr("data-type") +
        "]"
...
function buildEditElement(key, label, type, value, parentObj) {
  ...
  if (type == "input") {
    ...
    element =
      label + ' : <input type="checkbox" id="' + key + '" ' + valuetxt + " />";
  } else if (type == "textarea") {
    c = "";
    element = "<label>" + label + ' </label>  <textarea id="' + key + '">';
    if (
      parentObj.data("type") == "select" ||
      parentObj.data("type") == "radio" ||
      parentObj.data("type") == "checkbox"
    ) {
      if (key == "radios") {
...
function getValueTxt(key, parentObj, value) {
  ...
  if (key == "label") {
    ...
    if (parentObj.data("type") == "textarea") {
      valuetxt = parentObj.find("textarea").attr("name")
        ? parentObj.find("textarea").attr("name")
        : "";
    } else if (parentObj.data("type") == "select") {
      valuetxt = parentObj.find("select").attr("name")
        ? parentObj.find("select").attr("name")
        : "";
    } else {
      valuetxt = parentObj.find("input").attr("name")
        ? parentObj.find("input").attr("name")
        : "";
    }
  } else if (key == "placeholder") {
    if (parentObj.data("type") == "input") {
      valuetxt = parentObj.find("input").attr("placeholder")
        ? parentObj.find("input").attr("placeholder")
        : "";
    } else {
      valuetxt = parentObj.find("input").attr("name")
        ? parentObj.find("input").attr("name")
  ...
  $.ajax({
    ...
    success: function (response, status, request) {
      ...
      if (disp && disp.search("attachment") != -1) {
        var form = $(
          '<form method="POST" action="exportFormContentsCms.action">'
        );
