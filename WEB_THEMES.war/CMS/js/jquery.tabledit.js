    ...
    $.fn.Tabledit = function(options) {
        ...
        var Delete = {
            ...
            submit: function(td) {
                ...
                // Hide table row.
                $(td).parent('tr').addClass(settings.mutedClass+" "+ settings.dangerClass).find('.tabledit-toolbar button:not(.tabledit-restore-button)').attr('disabled', true);
                // Show restore button.
            ...
            restore: function(td) {
                ...
                // Hide table row.
                $(td).parent('tr').removeClass(settings.mutedClass+" "+ settings.dangerClass).find('.tabledit-toolbar button').attr('disabled', false);
                // Hide restore button.
        ...
        if (settings.editButton) {
            ...
            $(document).on('click', function(event) {
                ...
                if (!$editMode.is(event.target) && $editMode.has(event.target).length === 0) {
                	 ...
                	 var $input = $table.find('.tabledit-input:visible').parent('td').find('.tabledit-input');
                 //	console.log("Value : "+$input.val());
                	 if(typeof $input.val()!="undefined"){
                 	$table.find('.tabledit-input:visible').parent('td').find(".tabledit-span").html($input.val());
                }
