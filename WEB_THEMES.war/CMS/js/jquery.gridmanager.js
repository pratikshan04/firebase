...
(function ($) {
  ...
  $.gridmanager = function (el, options) {
          ...
          if (hasNested) {
            ...
            $.each($(col).contents(), function (i, val) {
              if ($(val).hasClass(gm.options.rowClass)) {
                try {
                  var prev = Array.prototype.reverse.call(
                      $(val).prevUntil("." + gm.options.rowClass)
                    ),
    ...
    gm.initDefaultButtons = function () {
      ...
      if (gm.options.editableRegionEnabled) {
        gm.options.customControls.global_col.push({
          ...
          iconClass: "cmsEditable",
          title: "Add Editable Region",
          btnClass: "cmsEditRegion",
        });
      }
    };
    ...
    gm.addResponsiveness = function (html) {
      ...
      return $(html)
        ...
        .each(function (i, el) {
          ...
          colNum = (colNum = desktopRegexObj.exec(elClasses))
            ? colNum[1]
            : (colNum = tabletRegexObj.exec(elClasses))
            ? colNum[1]
            : phoneRegexObj.exec(elClasses)[1];
    ...
    gm.createControls = function () {
      ...
      buttons.push(
        '<div class="row"><div id="layoutCarousel" class="col-sm-6 col-xs-12" style="position: relative; top: 0px; left: 0px; width:575px; height: 40px; overflow: hidden;">'
      );
      buttons.push(
        '<span u="arrowleft" class="layout03l" style="width: 55px; height: 55px; top: 123px; left: 8px;"></span>'
      );
      ...
      buttons.push(
        '<div u="slides" style="cursor: move; position: absolute; left:25px; top: 5px; width: 470px; height: 34px; overflow: hidden;">'
      );
      ...
      $.each(gm.options.controlButtons, function (i, val) {
        ...
        buttons.push(
          "<div><a title='Add Row " +
            _class +
            "' class='" +
            gm.options.controlButtonClass +
            " add" +
            _class +
            '\'><img u="image" src="../ASSETS/WEB_THEMES/CMS/images/cms_images/col_' +
            _class +
            '.png" width="40px" height="25px"  /></a></div>'
        );
        gm.generateClickHandler(val);
      });
      buttons.push("</div>");
      buttons.push(
        '<span u="arrowright" class="layout03r" style="width: 55px; height: 55px; top: 123px; right: 8px"></span>'
      );
      ...
      gm.$el.prepend(
        $("<div/>", {
          ...
          class: gm.options.gmClearClass + " " + "gmControlsCustomClass",
        }).prepend(
    ...
    gm.addCSS = function (myStylesLocation) {
      if (myStylesLocation !== "") {
        $('<link rel="stylesheet" href="' + myStylesLocation + '">').appendTo(
          "head"
        );
      }
    };
    ...
    gm.switchLayoutMode = function (mode, reInit) {
      ...
      temp_html = gm.cleanSubstring(
        gm.options.classRenameSuffix,
        temp_html,
        ""
      );
      ...
      temp_html = temp_html.replace(
        new RegExp(regex1 + '(?=[^"]*">)', "gm"),
        "$1" + gm.options.classRenameSuffix
      );
      temp_html = temp_html.replace(
        new RegExp(regex2 + '(?=[^"]*">)', "gm"),
        "$1" + gm.options.classRenameSuffix
      );
    ...
    gm.showEditor = function () {
      ...
      if (gm.mode === "visual") {
        gm.deinitCanvas();
        canvas.html(
          $("<textarea/>").attr("cols", 130).attr("rows", 25).val(canvas.html())
        );
        gm.mode = "html";
      ...
      gm.$el
        .on("click", ".gm-preview", function () {
          if (gm.status) {
            ...
            $(this)
              .html(
                "<span class='action-icon'><i class='fas fa-arrow-left fa-2x'></i></span><span class='action-text'>Back</span>"
              )
              .attr("title", "Back");
            gm.deinitCanvas();
            ...
            $(this)
              .html(
                "<span class='action-icon'><i class='fa fa-2x fa-file'></i><sup class='fas fa fa-eye'></sup></span><span class='action-text'>Page Preview</span>"
              )
              .attr("title", "Preview");
            gm.initCanvas();
        ...
        .on("click", ".gm-edit-mode", function () {
          if (gm.mode === "visual") {
            ...
            $(this)
              .parent()
              .find(".gm-preview, .gm-layout-mode > button")
              .prop("disabled", true);
            $(this)
              .parent()
              .find(".gm-preview, .gm-layout-mode > button")
            ...
            $(this)
              .html(
                "<span class='action-icon'><i class='fa fa-2x fa-file-code'></i></span><span class='action-text'>Source Code</span>"
              )
              .attr("title", "Edit Source Code");
            //CodeMirror.toTextArea();
            document.getElementById("sourceEditor").value = editor.getValue();
            var editedSource = canvas
              .find("textarea")
              .val()
              .replace(/\n/g, "")
              .replace(/  /g, "");
        ...
        .on(
          "click",
          "." + gm.options.gmEditRegion + " ." + gm.options.gmContentRegion,
          function () {
        ...
        .on("click", "a.gm-addRow", function () {
          ...
          $(this)
            .closest("." + gm.options.gmEditClass)
            .append(
              $("<div>")
                .addClass(gm.options.rowClass)
                .html(gm.createCol(6))
        ...
        .on("click", "a.gm-colDecrease", function () {
          ...
          if (t.colWidth > parseInt(gm.options.colResizeStep, 10)) {
            t.colWidth =
              parseInt(t.colWidth, 10) - parseInt(gm.options.colResizeStep, 10);
            //col.switchClass(t.colClass, gm.options.currentClassMode + t.colWidth, 200,triggerRize); //comment for device resize changes
            col
              .removeClass(t.colClass)
              .addClass(gm.options.currentClassMode + t.colWidth);
            canvas
              .find(gm.options.colSelector)
              .find("img")
        ...
        .on("click", ".gm-moveCol", function () {
          ...
          console.log(
            $(idx).children("div.gm-tools").find("a[data-tool1='editorTools']")
              .length
          );
          console.log(
            "Length : " +
              $(this)
                .closest(".gm-editing")
                .children("div.gm-tools")
                .find("a[data-tool1='editorTools']").length
          );
        ...
        .on("click", "a.gm-removeCol", function () {
          ...
          $(this)
            .closest("." + gm.options.gmEditClass)
            .animate(
              { opacity: "hide", width: "hide", height: "hide" },
              400,
              function () {
                $(this).remove();
              }
        ...
        .on("click", "a.gm-removeRow", function () {
          ...
          $(this)
            .closest("." + gm.options.gmEditClass)
            .animate({ opacity: "hide", height: "hide" }, 400, function () {
              $(this).remove();
              checkForContent();
        ...
        .on(
          "click",
          "a.gm-resetgrid, a.gm-remove, a.gm-removeRow, a.gm-save, button.gm-preview, a.gm-viewsource, a.gm-addColumn, a.gm-colDecrease, a.gm-colIncrease",
          function (e) {
            gm.log(
              "Clicked: " +
                $.grep(this.className.split(" "), function (v) {
    ...
    gm.initGlobalCustomControls = function () {
      ...
      $.each(["row", "col"], function (i, control_type) {
        if (
          typeof gm.options.customControls["global_" + control_type] !==
          "undefined"
        ) {
          elems = canvas.find(gm.options[control_type + "Selector"]);
          $.each(gm.options.customControls["global_" + control_type], function (
            i,
            curr_control
          ) {
            ...
            curr_control.dataToolElement =
              typeof curr_control.dataToolElement === "undefined"
                ? ""
                : curr_control.dataToolElement;
            curr_control.dataToolVal =
              typeof curr_control.dataToolVal === "undefined"
                ? ""
                : curr_control.dataToolVal;

            console.log("Custom : " + curr_control.dataToolElement);
            btnClass =
              typeof curr_control.callback === "function"
                ? i + "_btn"
                : curr_control.callback;
      ...
      $(
        ...
        for (prop in $(this).data()) {
          if (prop.indexOf("gmButton") === 0) {
            ...
            btnLoc =
              typeof callbackParams[3] !== "undefined"
                ? callbackParams[3]
                : "top";
            iconClass =
              typeof callbackParams[2] !== "undefined"
                ? callbackParams[2]
                : "fa fa-file-code-o";
            btnLabel =
              typeof callbackParams[1] !== "undefined" ? callbackParams[1] : "";
    ...
    gm.setupCustomBtn = function (
      ...
      $("." + gm.options.gmToolClass + btnLoc, container)
        .append(gm.buttonFactory([btnObj]))
        .find(":last")
        .on("click", function (e) {
    ...
    gm.isValidCallback = function (callbackScp, callbackFunc) {
      ...
      if (callbackScp === "window") {
        if (typeof window[callbackFunc] === "function") {
          ...
          // If the global function is not valid there is nothing to do
          return false;
        }
      } else if (typeof window[callbackScp][callbackFunc] === "function") {
        callback = window[callbackScp][callbackFunc];
      } else {
        // If there is no valid callback there is nothing to do
        return false;
      }
      return callback;
    };
    ...
    gm.activateContainer = function () {
      $("#mycanvas .container-fluid, #mycanvas  .container")
        .addClass(gm.options.gmEditClass)
        .prepend(gm.toolFactory(gm.options.containerButtonsPrepend));
    };
    /**
    ...
    gm.deactivateRows = function (rows) {
      ...
      rows
        .removeClass(gm.options.gmEditClass)
    ...
    gm.createRow = function (colWidths) {
      ...
      if (
        $("#containFluid").is(":checked") &&
        !$(canvas[0].firstElementChild).hasClass("container-fluid")
      ) {
        ...
        !$("#containFluid").is(":checked") &&
        !$(canvas[0].firstElementChild).hasClass("container")
      ) {
    ...
    gm.generateColSettings = function (col) {
      ...
      $.each(gm.options.colCustomClasses, function (i, val) {
        var btn = $("<button/>")
          ...
          .addClass(gm.options.controlButtonClass)
          .append($("<span/>").addClass(gm.options.controlButtonSpanClass))
          .append(" " + val);
        if (col.hasClass(val)) {
          btn.addClass(gm.options.gmDangerClass);
        }
    ...
    gm.addEditableAreaClick = function (container, btn) {
      ...
      $("." + gm.options.gmToolClass + ":last", container)
        .before(
          (elem = $("<div>")
            .addClass(
              gm.options.gmEditRegion + " " + gm.options.contentDraggableClass
            )
            .append(
              gm.options.controlContentElem +
                '<div class="' +
                gm.options.gmContentRegion +
    ...
    function showAllIcons() {
      jQuery(
        ".cimm_saveIcon,.cimm_publishIcon,.cimm_addNewIcon,.cimm_cmsResetIcon,.cimm_previewIcon,.cimm_srcCodeIcon"
      ).css({ "pointer-events": "visible", opacity: "1" });
    }
    gm.initNewContentElem = function (newElem) {
      ...
      var dropOpts = {
        drop: function (event, ui) {
          ...
          var content = $(ui.draggable)
            .clone()
            .find("div.hiddenContent")
            .html();
          var target = $(this).find("." + gm.options.gmContentRegion);
          if (
            target.children("p").text() != null &&
            typeof target.children("p").text() != "undefined" &&
            jQuery.trim(target.children("p").text()) == "Enter Content Here"
          )
          ...
          var sliderVar = "";
          var sliderWrapper = "";
          var contentDataAttr = jQuery(content).data("type");
          console.log("contentDataAttr : " + contentDataAttr);
          if (
            typeof content != "undefined" &&
            content.indexOf("slidercontainer") != -1
          ) {
            $(this)
              .find("." + gm.options.gmContentRegion)
              .attr({ "data-name": "bannerBlock" });
            $(this)
              .find("." + gm.options.gmContentRegion)
              .addClass("bannerBlock");
            var slideCounter = $("div[id$='_Wrapper']").length;
            sliderVar = "slider" + slideCounter + "_container";
            sliderWrapper = sliderVar + "_Wrapper";
            console.log("true");
            initContextMenu("#" + sliderVar);
            content = content.replace(
              "slidercontainer",
              "slider" + slideCounter + "_container"
            );
            content = "<div id='" + sliderWrapper + "'>" + content + "</div>";
            loadSlider = true;
            // $("#slideCounter").val(parseInt(slideCounter)+1);
          }

          if (contentDataAttr == "widget") {
            $(this)
              .find("." + gm.options.gmContentRegion)
              .attr({ "data-name": "widgetBlock" });
          ...
          if (contentDataAttr == "form") {
            $(this)
              .find("." + gm.options.gmContentRegion)
              .attr({ "data-name": "formBlock" });
            jQuery(content).data("select", "form");
            jQuery(content).attr("data-select", "form");
          }

          $(content).appendTo(target);
          if (contentDataAttr == "widget") {
            var widgetId = jQuery(content).data("widget");
            jQuery("[data-widget='" + widgetId + "']").attr({
              "data-select": "widget",
            });
          ...
          if (contentDataAttr == "form") {
            ...
            jQuery("[data-widget='" + formId + "']").attr({
              "data-select": "form",
            });
            generateForm(formId);
          }
          ...
          $("#mycanvas a[href]").click(function (e) {
            e.preventDefault();
            if (
              $(this).attr("href").indexOf("cmsStaticPageByPageNameCms") >= 0
            ) {
              var url = $(this).attr("href");
              $.ajax({
                type: "GET",
      ...
      var sortOpts = {
        items:
          "." +
          gm.options.contentDraggableClass +
          " ." +
          gm.options.contentDroppableClass,
        axis: "y",
        placeholder: gm.options.rowSortingClass,
      ...
      $.each(parentCols, function (i, col) {
        $(col).on("click", ".gm-delete", function (e) {
          $(this)
            .closest("." + gm.options.gmEditRegion)
            .remove();
    ...
    gm.buttonFactory = function (btns) {
      ...
      $.each(btns, function (i, val) {
        ...
        val.dataToolElement =
          typeof val.dataToolElement === "undefined" ? "" : val.dataToolElement;
        val.dataToolVal =
          typeof val.dataToolVal === "undefined" ? "" : val.dataToolVal;
        bt = "";
        btnClass = "";
        if (val.title == "Add Editable Region") btnClass = val.btnClass;
        else btnClass = val.btnClass;
      ...
      //jQuery(".gm-rowSettings").trigger("click");
      return buttons.join("");
    };
        ...
        case "attach":
          switch (gm.options.rte) {
            case "tinymce":
              ...
              if (element.data("name") == "bannerBlock") {
                enableEditor = false;
              }
              if (element.data("name") == "widgetBlock") {
                element.attr("contenteditable", "false");
                enableEditor = false;
              }
              if (element.data("name") == "formBlock") {
                element.attr("contenteditable", "false");
                enableEditor = false;
              }
              ...
              if (enableEditor) {
                if (
                  element.children("p").text() != null &&
                  typeof element.children("p").text() != "undefined" &&
                  jQuery.trim(element.children("p").text()) ==
                    "Enter Content Here"
                )
    ...
    gm.cleanup = function () {
       ...
       */
      content =
        gm.mode !== "visual" ? canvas.find("textarea").val() : canvas.html();
  ...
  */
    ...
    tinymce: {
      config: {
        ...
        toolbar1:
          " bold italic underline strikethrough | removeformat |  styleselect formatselect fontselect fontsizeselect  ",
        toolbar2:
          "cut copy | searchreplace | bullist numlist | outdent indent blockquote | undo redo | link unlink image media |  table ",
        toolbar3:
          "insertdatetime | subscript superscript | alignleft aligncenter alignright alignjustify | charmap emoticons | fullscreen |  forecolor backcolor | code | fontawesome | closeBtn",
  ...
  hideActionIcons = function (clickType) {
    ...
    if (clickType == "preview") {
      ...
      jQuery(".Cimm-MainContent")
        .addClass("Cimm-MainContenthide")
        .removeClass("Cimm-MainContent");
  ...
  showActionIcons = function (clickType) {
    ...
    if (clickType == "preview") {
      ...
      jQuery(".Cimm-MainContenthide")
        .addClass("Cimm-MainContent")
        .removeClass("Cimm-MainContenthide");
